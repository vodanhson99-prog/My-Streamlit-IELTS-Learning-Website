...
import type { IeltsBand, IeltsTask2CriterionId, RubricCriterion, Task2Rubric } from "../contracts"

const ZERO_DESCRIPTOR =
...
const descriptors: Record<IeltsTask2CriterionId, Record<IeltsBand, string>> = {
  "task-response": {
...
const criterionNames: Record<IeltsTask2CriterionId, string> = {
...
const criteria = IELTS_TASK2_CRITERION_IDS.map((criterionId): RubricCriterion => ({
  id: criterionId,
  name: criterionNames[criterionId],
  bands: IELTS_BANDS.map((band) => ({
    id: `${IELTS_TASK2_RUBRIC_VERSION}.${criterionId}.band-${band}`,
    band,
    descriptor: descriptors[criterionId][band],
  })),
}))

export const IELTS_TASK2_RUBRIC = {
  id: "ielts-writing-task-2",
  version: IELTS_TASK2_RUBRIC_VERSION,
  publishedDate: IELTS_TASK2_RUBRIC_PUBLISHED_DATE,
  sourceUrl: IELTS_TASK2_RUBRIC_SOURCE_URL,
  criteria,
} as const satisfies Task2Rubric
