...
export function buildCriterionPrompt(input: CriterionPromptInput) {
  const system = [
    ...
    `Required JSON shape: {"criterionId":"${input.criterionId}","band":<integer 0-9>,"descriptorId":"<matching supplied descriptor id>","supportingEvidence":[{"anchor":{"type":"span","quote":"<exact essay quote>"},"rationale":"<descriptor-grounded reason>"}],"limitingEvidence":[{"anchor":{"type":"paragraph","paragraphIndex":<zero-based integer>},"rationale":"<why next descriptor is not sufficiently supported>"}],"nextBandBlockers":["<reason next band is not supported>"],"annotationCandidates":[{"quote":"<exact essay quote>","label":"<allowed label>","rationale":"<criterion-specific reason>"}]}. supportingEvidence requires at least one item. Below Band 9, limitingEvidence and nextBandBlockers each require at least one item; at Band 9 they may be empty.`,
  ].join("\n")
